...
class Timer extends React.Component {
  ...
  componentDidMount() {
  let sec =  new Date(Date.parse(this.props.timerdata) - Date.parse(new Date()))
  let final =  sec.getTime() / 1000
   let timeLeftVar = this.secondsToTime( final);
   this.setState({ time: timeLeftVar, seconds: final });
   if (this.timer === 0) {
...
{sec > 0 ? 
      <React.Fragment>
            {/* <div className="timer-nos">
              <button >Start</button>
              m: {this.state.time.m} s: {this.state.time.s}
            </div> */}
            <h1 className="text-center">Item will disappear</h1>
